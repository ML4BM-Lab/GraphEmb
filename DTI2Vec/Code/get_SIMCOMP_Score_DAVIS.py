...
def main():
    ...
    # DB_PATH =  '/home/margaret/data/jfuente/DTI/Input4Models/DB/Data//Davis_et_al/Davis_30.tsv'
    logging.info(f'Reading database from: {DB_PATH}')
    db_name = hf.get_DB_name(DB_PATH)

    # read the DB and keep the drugs
    drugs = hf.read_and_extract_DAVIS(DB_PATH)
    drugs = list(set(drugs))
    logging.info(f'{len(drugs)} drugs found')

    drugs = list(set(drugs))
    drugs_smiles = hf.get_SMILES_from_Pubchem_web_batch(drugs)
