...
def main():
	...
	encoded_dtis = [ (node_index_dict.get(node1), node_index_dict.get(node2)) for node1, node2 in dtis ]
	...
	hf.write_dict(node_index_dict,  os.path.splitext(dti_coded_path)[0] + '_node_index_dict.txt')
	...
	for dim in tqdm(dimensions, desc='Creating the embeddings'):
		embeddings_file  = f'./../Data/{db_name}/{subname}_Nodes_Embedding_{str(dim)}.emb'
		...
		if out_1 == 0:
			...
			logging.error(f'Something went wrong with the node2vec implementation. Check the output of the command: {cmd}')
