...
def main():
    ...
    parser.add_argument(
        ...
        default="./../../DB/Data/Yamanashi_et_al_GoldStandard/E/interactions/e_admat_dgc_mat_2_line.txt",
        type=str,
    )
    parser.add_argument(
        "-v",
    ...
    for K_neigh in [2, 5, 10, 20]:
        ...
        file_path = os.path.join('./../Data',db_name , hf.get_yamanashi_subDB(db_name) + '_' +str(K_neigh) )
        ...
        for cv in range(10):
            ...
            for dim in tqdm(dimensions, desc="Creating the embeddings"):
                embeddings_file = (
                    f"./../Data/{db_name}/{subname}_Nodes_Embedding_{str(dim)}_{K_neigh}_{cv}.emb"
                )
