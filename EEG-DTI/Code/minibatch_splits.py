...
class EdgeMinibatchIterator(object):
    ...
    """
    def __init__(self, adj_mats, feat, edge_types, batch_size=100, data_set='luo', val_test_size=0.01, fold = 0, seed = 0):
    ...
    def mask_test_edges(self, edge_type, type_idx, fold):
        if edge_type==(0,1) or edge_type==(1,0):
            ...
            train_edges = np.loadtxt('./'+data_folder+'/'+ratio+'/train_index_(0, 1)'+str(seed)+'_'+str(fold)+'.txt',dtype=int)
            if edge_type == (1, 0):
            ...
            test_edges_false = np.loadtxt('./'+data_folder+'/'+ratio+'/index_test_false(0, 1)'+str(fold)+'.txt',dtype=int)
            if edge_type == (1,0):
            ...
            test_edges = np.loadtxt('./'+data_folder+'/'+ratio+'/test_index_(0, 1)'+str(fold)+'.txt',dtype=int)
            if edge_type == (1, 0):
