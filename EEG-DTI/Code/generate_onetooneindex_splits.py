...
def main():
	...
	# create folders
	path_folder = os.path.join(wdir, f'oneTooneIndex_{SPLIT_TYPE}_{sub}{rmsd_name}')
	...
	# Generate splits
	splits_gen = generate_splits(DTIs, mode= SPLIT_TYPE, subsampling=SUBSAMPLING_TYPE, foldnum=10,  RMSD_dict=RMSD_SET)
	# this also changes (drug, protein) to (protein, drug)
	splits = get_id2idx_protdrug(wdir, splits_gen)
	nseed, nfold = 0, 0 
	for nseed, nfold in product(range(len(splits)), range(len(splits[nseed]))):
		np.savetxt(os.path.join(path_folder, f'train_index_pos(0,1){nseed}_{nfold}.txt'), splits[nseed][nfold][0], fmt='%i', delimiter=" ")
		np.savetxt(os.path.join(path_folder, f'train_index_neg(0,1){nseed}_{nfold}.txt'), splits[nseed][nfold][1], fmt='%i', delimiter=" ")
		np.savetxt(os.path.join(path_folder, f'test_index_pos(0,1){nseed}_{nfold}.txt'), splits[nseed][nfold][2], fmt='%i', delimiter=" ")
		np.savetxt(os.path.join(path_folder, f'test_index_neg(0,1){nseed}_{nfold}.txt'), splits[nseed][nfold][3], fmt='%i', delimiter=" ")
