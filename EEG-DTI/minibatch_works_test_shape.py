...
class EdgeMinibatchIterator(object):
    ...
    def mask_test_edges(self, edge_type, type_idx, fold):
        if edge_type==(0,1) or edge_type==(1,0):
            ...
            test_edges = np.loadtxt('./'+data_folder+'/'+ratio+'/test_index_pos(0,1)'+str(seed)+'_'+str(fold)+'.txt',dtype=int)
            if edge_type == (1, 0):
            ...
            adj_train = sp.csr_matrix(
                (data, (train_edges[:, 0], train_edges[:, 1])), shape=self.adj_mats[edge_type][type_idx].shape)
