...
def main():
	...
	if os.path.isfile(annotation_file):
		...
		root = tree.getroot()
		drug_bank_db_loaded = True
		# db_drugProd_drugId
		pubchem_db_ID_dict = {}
		name_DbId_dic = {} # dict with generic or product name as key and drugbank_id as value
		for drug_entry in tqdm(root):
			...
			if name:
				name_DbId_dic[name] = drugbank_ID
			if len(prod_names) >= 1:
				for prod in prod_names:
					name_DbId_dic[prod] = drugbank_ID
		blacklist = ['TABLETS', 'CAPSULES', 'INJECTABLE', 'INJECTION', 'MG', 'VIAL', 'ML']
		fda_DB_dict = []
		for fda_id, drug_name in tqdm(all_drugs):
			if name_DbId_dic.get(drug_name, None):
				fda_DB_dict.append((fda_id, drug_name, name_DbId_dic.get(drug_name)))
			else:
				...
				if name_DbId_dic.get(drug_name_cleaned, None):
					fda_DB_dict.append((fda_id, drug_name_cleaned, name_DbId_dic.get(drug_name_cleaned)))
 ...
 // TODOnjmake it work
	...
	if drug_bank_db_loaded:
		db_puchem_dict = hf.getDB_from_binding(drugs, root)
	else:
		db_puchem_dict = hf.getDB_from_binding(drugs)
	...
	for drug in tqdm(drugs):
		drug_db_equivalent = db_puchem_dict.get(drug, None)
...
a =  hf.get_drugNames_from_Pubchem_web(drugs[50:100])