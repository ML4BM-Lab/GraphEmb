...
def main():
    ...
    if os.path.isfile(annotation_file):
        ...
        assert (
            len(set([len(event) for event in fda_kegg_dict])) == 1
        ...
        for entry in tqdm(kegg_drug_names):
            ...
            parsed_desc = [
                KEGG_NAME_DB.findall(pseudo.strip()) for pseudo in kegg_desc.split(";")
            ]
        ...
        for fda_id, drug_name in tqdm(all_drugs):
            ...
            if kegg_id:
                fda_kegg_dict.append(
                    (fda_id.strip(), drug_name.strip(), kegg_id.strip())
                )
            else:
                ...
                if kegg_id:
                    ...
                    fda_kegg_dict.append(
                        (fda_id.strip(), drug_name_cleaned.strip(), kegg_id)
                    )
