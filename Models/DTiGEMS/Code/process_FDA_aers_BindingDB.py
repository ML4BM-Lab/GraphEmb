...
def main():
    ...
    if os.path.isfile(annotation_file):
        ...
        fda_DB_dict = []
        for fda_id, drug_name in tqdm(
            all_drugs, desc="Getting names from FDA to Pubchem"
        ):
            if db_puchem_dict_inv.get(drug_name.upper(), None):
                fda_DB_dict.append(
                    (fda_id, drug_name, db_puchem_dict_inv.get(drug_name.upper())[0])
                )
            else:
                ...
                if db_puchem_dict_inv.get(drug_name_cleaned.upper(), None):
                    fda_DB_dict.append(
                        (
                            fda_id,
                            drug_name_cleaned,
                            db_puchem_dict_inv.get(drug_name_cleaned.upper())[0],
                        )
                    )
    ...
    all_events = [
        event
        for event in tqdm(all_events, desc="Keeping the events for the selected drugs")
    ...
    aers_freq_pd.to_csv(
        os.path.join(path, "BindingDB_Drug_FDA_aers_freq.tsv"), sep="\t"
    )
