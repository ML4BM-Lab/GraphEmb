function [roc_avg, pr_avg] = DTINet(seed, nFold, interaction, drug_feat, prot_feat, dim_imc)
	...
	%
	sint = size(interaction);
	ntot_prots = sint(2);
	% num negs
	[row, col] = ind2sub(size(interaction), Pnoint);
	% col for proteins
	prots_neg_see = length(unique(col));
	fprintf('Percentage seen negative %.2f \n', prots_neg_see/ntot_prots * 100);
