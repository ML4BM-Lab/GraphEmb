...
import Drugs_AERS_FDA_SIDER_helpers as hf


def BINDINGDB_SIDER(DB_PATH = './DB/Data/BindingDB/tdc_package_preprocessing/BindingDB_max_affinity.tsv'):
...
def DAVIS_SIDER(DB_PATH = './DB/Data/Davis_et_al/tdc_package_preprocessing/DAVIS_et_al.tsv'):
...
def BIOSNAP_SIDER(DB_PATH =  './DB/Data/BIOSNAP/ChG-Miner_miner-chem-gene/ChG-Miner_miner-chem-gene.tsv'):
...
def DRUGBANK_SIDER(DB_PATH =  './DB/Data/DrugBank/DrugBank_DTIs.tsv'):
	...
	pubchem_to_stitch = hf.get_STITCH_from_Pubchem(set(drugBank_to_pbC.values()), set(drugBank_to_pbS.values()))

	drug_side_effects = []
	all_side_effects = []
	CID_PATTERN = re.compile(r'(?<=CID[ms]{1})[\d]+')
	for drug in tqdm(drugs, desc='Assigning Side Effects to drugs'):
		pubchem_id = drugBank_to_pbC.get(drug,None) if drug in drugBank_to_pbC else drugBank_to_pbS.get(drug,None) 
		stitch = pubchem_to_stitch.get(pubchem_id, None)
		if stitch:
			stitch = CID_PATTERN.search(stitch).group()
			sider_SE = sider_SE_dict.get(stitch)
...
def YAMANASHI_SIDER(DB_PATH =  './DB/Data/Yamanashi_et_al_GoldStandard/NR/interactions/nr_admat_dgc_mat_2_line.txt'):
	fmt = '[%(levelname)s] %(message)s'
	logging.basicConfig(format=fmt, level=logging.DEBUG)
