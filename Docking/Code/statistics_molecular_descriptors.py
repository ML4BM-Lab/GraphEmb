...
drugs_yamanishi = get_drugs_yamanishi() # need to change for dict now !
...
def calculate_descriptor(mol_descr, list_datasets):
    ...
    for database in list_datasets:
        ...
        list_drug_smiles_inch = get_SMILES_Pubchem_web_batch(drugs_pubchem) 
        ...
        drug_descriptor = [f(fp) for fp in drug_mol]

        df[database] = pd.Series(drug_descriptor)
...
list_datasets = ['drugbank', 'biosnap', 'bindingdb', 'davis']


df = calculate_descriptor(mol_descr, list_datasets)
...
plt.savefig('test_boxplot.pdf', dpi=330 ,bbox_inches='tight',  pad_inches = 0.25)
